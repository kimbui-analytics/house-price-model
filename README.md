...
Business Question
    What property characteristics most influence housing sale price, and can we build a stable, generalizable pricing model that supports decision-making without overfitting to historical noise?
    The goal was not only to predict prices, but to understand which factors consistently drive home value.
Dataset
...
🔎 Key Insights
    ...
    ![Predicted vs Actual Sale Price](reports/figures/predicted_vs_actual_ridge.png)
    Model Error Analysis:
    Prediction errors are largest for high-end homes. Extremely expensive properties are inherently harder to price precisely and contribute disproportionately to total error.
    No systematic pricing bias was detected. The model does not consistently over- or under-estimate across the test set.
    ![Residual Distribution (Ridge)](reports/figures/residual_distribution_ridge.png)
    ...
    ![Top Drivers](reports/figures/top_drivers_ridge_bar.png)
    ...
    ![Ridge vs Lasso Comparison Plot)](reports/figures/ridge_vs_lasso_coefficients_top20.png)
